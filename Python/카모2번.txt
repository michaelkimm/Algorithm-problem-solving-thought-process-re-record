...
def getTimeByRide(ride_times, walk_times, m):
    ...
    distance_conseq_walk_zero = [INF for _ in range(len(ride_times))]
    hp = []
    if ride_times[0] != -1:
        heapq.heappush(hp, (ride_times[0], 0, 0))
        distance_conseq_walk_zero[0] = ride_times[0]
    ...
    while hp:
        ...
        # distance에 의한 조건 처리
        if distance_conseq_walk_zero[last_walked_road] < cur_consumed_time:
            continue
        ...
        if ride_times[next_road] != -1:
            ...
            if next_consumed_time < distance_conseq_walk_zero[next_road]:
                distance_conseq_walk_zero[next_road] = next_consumed_time
